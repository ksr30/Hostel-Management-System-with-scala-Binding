...
package com.knoldus {
  package Main {
    final case class Money_Transfer_Agreement(issuer: ` lfdomainapi`.Primitive.Party, owner: ` lfdomainapi`.Primitive.Party, money: ` lfdomainapi`.Primitive.Int64, signatories: ` lfdomainapi`.Primitive.List[` lfdomainapi`.Primitive.Party]) extends ` lfdomainapi`.Template[Money_Transfer_Agreement] {
      override protected[this] def templateCompanion(implicit ` d` : _root_.scala.Predef.DummyImplicit) = Money_Transfer_Agreement
    }

    object Money_Transfer_Agreement extends ` lfdomainapi`.TemplateCompanion[Money_Transfer_Agreement] with _root_.scala.Function4[` lfdomainapi`.Primitive.Party, ` lfdomainapi`.Primitive.Party, ` lfdomainapi`.Primitive.Int64, ` lfdomainapi`.Primitive.List[` lfdomainapi`.Primitive.Party], _root_.com.knoldus.Main.Money_Transfer_Agreement] {
      ...
      override val id = ` templateId`(packageId = `Package IDs`.Main, moduleName = "Main", entityName = "Money_Transfer_Agreement");
      final implicit class `Money_Transfer_Agreement syntax`[+` ExOn`](private val id: ` ExOn`) extends _root_.scala.AnyVal {
        def exerciseArchive(actor: ` lfdomainapi`.Primitive.Party, choiceArgument: com.knoldus.DA.Internal.Template.Archive)(implicit ` exOn` : ` lfdomainapi`.encoding.ExerciseOn[` ExOn`, Money_Transfer_Agreement]): ` lfdomainapi`.Primitive.Update[` lfdomainapi`.Primitive.Unit] = ` exercise`(id, "Archive", _root_.scala.Some(` lfdomainapi`.Value.encode(choiceArgument)));
        def exerciseArchive(actor: ` lfdomainapi`.Primitive.Party)(implicit ` exOn` : ` lfdomainapi`.encoding.ExerciseOn[` ExOn`, Money_Transfer_Agreement]): ` lfdomainapi`.Primitive.Update[` lfdomainapi`.Primitive.Unit] = exerciseArchive(actor, _root_.com.knoldus.DA.Internal.Template.Archive())
      };
      override val consumingChoices: Set[` lfdomainapi`.Primitive.ChoiceId] = ` lfdomainapi`.Primitive.ChoiceId.subst(Set("Archive"));
      override def toNamedArguments(` self` : Money_Transfer_Agreement) = ` arguments`(scala.Tuple2("issuer", ` lfdomainapi`.Value.encode(` self`.issuer)), scala.Tuple2("owner", ` lfdomainapi`.Value.encode(` self`.owner)), scala.Tuple2("money", ` lfdomainapi`.Value.encode(` self`.money)), scala.Tuple2("signatories", ` lfdomainapi`.Value.encode(` self`.signatories)));
                  ...
                  case ` rpcvalue`.RecordField((""| "money"), _root_.scala.Some(zv2)) => (` lfdomainapi`.Value.decode[` lfdomainapi`.Primitive.Int64](zv2) match {
                    case _root_.scala.Some(z2) => (` r`.fields(3) match {
                      case ` rpcvalue`.RecordField((""| "signatories"), _root_.scala.Some(zv3)) => (` lfdomainapi`.Value.decode[` lfdomainapi`.Primitive.List[` lfdomainapi`.Primitive.Party]](zv3) match {
                        case _root_.scala.Some(z3) => Some(Money_Transfer_Agreement(z0, z1, z2, z3))
      ...
      override def encoding(lte: ` lfdomainapi`.encoding.LfTypeEncoding)(`view `: view[lte.Field]): lte.Out[_root_.com.knoldus.Main.Money_Transfer_Agreement] = {
        val `recordFields `: lte.RecordFields[_root_.com.knoldus.Main.Money_Transfer_Agreement] = lte.RecordFields.xmapN(lte.fields(`view `.issuer), lte.fields(`view `.owner), lte.fields(`view `.money), lte.fields(`view `.signatories))({
          case scala.Tuple4(issuer, owner, money, signatories) => _root_.com.knoldus.Main.Money_Transfer_Agreement(issuer, owner, money, signatories)
        })({
          case _root_.com.knoldus.Main.Money_Transfer_Agreement(issuer, owner, money, signatories) => scala.Tuple4(issuer, owner, money, signatories)
        });
